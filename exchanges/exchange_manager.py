    ...
    def process_markets(self):
        try:
            if self.market_type == "option":
                ...
                markets = [market for market in filtered_markets if market.get(
                ...
                for market in filtered_markets:
                    ...
                    time_to_maturity_days = (
                        expiration_date - current_date).days
                ...
                if most_common_expiry:
                    ...
                    filtered_data = [data for data in filtered_data if datetime.strptime(
                    ...
                    if min_diff_strike:
                        ...
                        if 'bids' in option_order_books_data.get('order_book', {}) and 'asks' in option_order_books_data.get('order_book', {}):
                            bids, asks = option_order_books_data['order_book']['bids'], option_order_books_data['order_book']['asks']
                        ...
                        if implied_forward_price > 0:
                            ...
                            largest_strike = 0
                            for bid in bids:
                                if len(bid) >= 2:  # Ensure bid has at least two elements (price and quantity)
                                    bid_strike = float(bid[0])
                                    print(f"Checking bid_strike: {bid_strike}, implied_forward_price: {implied_forward_price}, largest_strike: {largest_strike}")
                                    if bid_strike < implied_forward_price and bid_strike > largest_strike:
                                        largest_strike = bid_strike
                                        print(f"Updating largest_strike: {largest_strike}")

                            if largest_strike > 0:
                                print(f"Largest Strike (KATM) for {symbol}: {largest_strike}")
                            else:
                    ...
                    self.data_saver.save_data(filtered_data, filename="filtered_data.json", exchange_name=self.exchange_name)
