...
class ExchangeManager:
    def __init__(
        self, exchange_name: str, symbol_filter: Optional[str], market_type: str
    ):
        self.exchange = getattr(ccxt, exchange_name)()
        self.symbol_filter = symbol_filter
        self.exchange_name = exchange_name
        self.market_type = market_type
        self.data: Dict[str, Dict[str, Union[List[float], List[float], float]]] = {}
    ...
    def initialize_exchange(self):
        try:
            ...
            self._handle_error(f"Error initializing exchange '{self.exchange_name}'", e)
    ...
    def _fetch_mark_price(self, symbol: str):
        try:
            mark_price = self._fetch_price(symbol, "markPrice") or self._fetch_price(
                symbol, "mark_price"
            )
            ...
            bid, ask = self._fetch_price(symbol, "bid"), self._fetch_price(
                symbol, "ask"
            )
    ...
    def _standardize_data(self, symbol: str, order_book_data: dict) -> dict:
        try:
            ...
            mark_price = (
                ...
                else self._fetch_mark_price(symbol)
            )
    ...
    def save_data_to_file(
        self, exchange_name: str, data: dict, filename: str = "data.txt"
    ):
        try:
            ...
            exchange_filename = os.path.join(folder_name, f"{exchange_name}_{filename}")

            if not data:
                logging.warning(f"Skipping save: Empty data for {exchange_name}")
                return
            ...
            logging.info(f"Data for {exchange_name} saved to {exchange_filename}")
        except Exception as e:
    ...
    def process_yield_curve(self):
        try:
            ...
            for symbol, interest_rates in yield_curve.items():
                average_interest_rate = sum(interest_rates) / len(interest_rates)
                average_yield_curve[symbol] = average_interest_rate

            logger.info(f"Yield Curve for {self.exchange_name}:\n{average_yield_curve}")
        ...
        try:
            ...
            if self.market_type == "option":
                ...
                for market in markets:
                    ...
                    option_order_books_data = self.fetch_option_order_books(symbol)
                ...
                self.save_data_to_file(self.exchange_name, future_order_books_data)
